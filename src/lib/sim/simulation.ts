...
import { computeBodyStats } from "./stats";
...
export class Simulation {
  ...
  private hudClock = 0;
  private noiseT = 0;
  private speedHist = new Float32Array(SPEED_HIST);
  private speedHistLen = 0;
  private speedHistHead = 0;
  private speedHistId: number | null = null;
  ...
  select(id: number | null): void {
    this.selectedId = id;
    if (id == null) this.followId = null;
    if (id !== this.speedHistId) this.resetSpeedHist();
    this.pushHud();
  }
  ...
  private resetSpeedHist(): void {
    this.speedHistLen = 0;
    this.speedHistHead = 0;
    this.speedHistId = this.selectedId;
  }

  private sampleSpeed(body: Body): number[] {
    if (this.speedHistId !== body.id) {
      this.speedHistId = body.id;
      this.speedHistLen = 0;
      this.speedHistHead = 0;
    }
    this.speedHist[this.speedHistHead] = Math.hypot(body.vx, body.vy);
    this.speedHistHead = (this.speedHistHead + 1) % SPEED_HIST;
    if (this.speedHistLen < SPEED_HIST) this.speedHistLen += 1;
    const hist: number[] = [];
    for (let i = 0; i < this.speedHistLen; i++) {
      const idx = (this.speedHistHead - this.speedHistLen + i + SPEED_HIST) % SPEED_HIST;
      hist.push(this.speedHist[idx]!);
    }
    return hist;
  }
  ...
  private pushHud(): void {
    const selectedBody =
      this.selectedId != null ? this.bodies.find((b) => b.id === this.selectedId) : null;
    const history = selectedBody ? this.sampleSpeed(selectedBody) : [];
    useSimStore.getState().patch({
      ...
      selected: selectedBody ? computeBodyStats(selectedBody, this.bodies, history) : null,
    });
  }
  ...
  private hitBody(wx: number, wy: number): Body | null {
    const slop = 22 / this.camera.zoom;
    let best: Body | null = null;
    let bestScore = Infinity;
    for (const b of this.bodies) {
      const d = Math.hypot(b.x - wx, b.y - wy);
      if (d > b.radius + slop) continue;
      const score = d / Math.max(b.radius, 1);
      if (score < bestScore) {
        best = b;
        bestScore = score;
      }
  ...
  private stepPhysics(dt: number): void {
    ...
    if (this.selectedId != null && !this.bodies.some((b) => b.id === this.selectedId)) {
      const last = events[events.length - 1];
      this.selectedId = last?.id ?? this.followId;
    }
  ...
  if (typeof window !== "undefined") {
    (window as Window & { __kepler?: unknown }).__kepler = sim
      ? {
          ...
          select: (id: number | null) => sim.select(id),
          bodies: () =>
            sim.bodies.map((b) => ({ id: b.id, kind: b.kind, x: b.x, y: b.y, r: b.radius })),
        }
      : undefined;
  }
}
