...
`main` is the stable branch. Do not commit to it directly. Every change — docs, features, fixes — goes through its own branch and a pull request.
...
The first export and the license/README commits were made on `main` before this flow existed. From here on, new work uses the steps above.
...
Use the PR template. Say **what** changed and **how to try it**. One concern per PR is enough.
